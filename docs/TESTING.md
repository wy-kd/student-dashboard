...
## V2.1 regression coverage

`tests/v21-client.test.ts` covers per-browser sidebar preference persistence, quiet idle long polling, visibility cancellation/reconnection, out-of-order reply rejection, session-expiry stop, connection timeout/backoff, owner-scoped event wakes and bounded wait cleanup. `tests/v2.test.ts` adds two-client authenticated timer transitions, stale-action rejection, held-request logout, recurrence deletion with future cleanup and completed/overdue/edited/started/timer-linked history preservation. Unknown `plan.save` requests fail without creating sessions; manual study and Weekly Review still work.

The production lifecycle integration observes remote start and pause through the built Next.js `/api/timer` route, checks pause delivery within two seconds, and keeps a timer request open during graceful stop. It restarts and checks the persisted paused timer and existing database/authentication/backup guards. The timer channel releases held reads on shutdown so they do not prolong draining; the unchanged launcher allows 60 seconds and does not force-kill.

Visual QA uses the real navigation, timer and recurrence components in the fictional fixture, including 390px phone, 820px portrait, 1180px landscape and desktop widths, both themes and density settings. Native browser fullscreen is unavailable in the review environment, so fallback entry, on-screen exit and Escape are checked there; native fullscreen, safe areas and real touch/keyboard behaviour require device acceptance. Generated fixture assets are ignored and removed before final delivery. No physical Acer reboot or iPhone/iPad test is claimed.
